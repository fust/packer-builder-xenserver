...
func (self *StepGetVNCPort) Run(state multistep.StateBag) multistep.StepAction {
	...
	cmd1 := fmt.Sprintf("nohup socat -d -d -lf /tmp/socat-%s TCP-LISTEN:%s,reuseaddr,fork,tcpwrap=socat,allow-table=all UNIX-CONNECT:/var/run/xen/vnc-%s &>/dev/null &", expectedport, expectedport, domid)
	ui.Say(fmt.Sprintf("nohup socat -d -d -lf /tmp/socat-%s TCP-LISTEN:%s,reuseaddr,fork,tcpwrap=socat,allow-table=all UNIX-CONNECT:/var/run/xen/vnc-%s &>/dev/null &", expectedport, expectedport, domid))
